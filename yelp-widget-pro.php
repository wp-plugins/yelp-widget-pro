...
if($options['yelp_widget_premium_license_status'] == "1") {
    add_action('init', 'yelp_widget_premium_activate');
    yelp_widget_premium_activate();

}

function yelp_widget_premium_activate() {
    require_once ('lib/wp_autoupdate.php');
    $yelp_widget_premium_current_version = '1.1';
    $yelp_widget_premium_remote_path = 'http://wordimpressed.local/downloads/update.php';
    $yelp_widget_premium_slug = plugin_basename(__FILE__);
    new wp_auto_update($yelp_widget_premium_current_version, $yelp_widget_premium_remote_path, $yelp_widget_premium_slug);
}
